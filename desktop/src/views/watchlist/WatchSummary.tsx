 ...
 *
 * Reads useMockWatchlist() from the shared store directly. Computes stat
 * tiles from the current items array (no derived data crosses the wire here).
 *
 * Stats rendered:
 *   - Total items
 *   - Active count
 ...
 *
 * "Add card or set" button opens the AddFlow modal via the shared store's
 * openAddFlow() — no prop-drilling.
 */
...
export function WatchSummary() {
  const { items, openAddFlow } = useMockWatchlist();
